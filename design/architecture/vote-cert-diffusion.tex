...
We propose to add an appropriate configuration flag to control this behavior, either causing the node to always request votes, or to only request votes if the node has inbound peers, or to not request votes at all (downloading just certificates instead).
