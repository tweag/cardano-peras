...
\newcommand{\StObjIdsBlockingR}    {\state{StObjIds Blocking MustReply}}
\newcommand{\StObjIdsBlockingA}    {\state{StObjIds Blocking CanAwait}}
...
\begin{figure}[h]
  \begin{center}
    \begin{tikzpicture}[->,shorten >=1pt,auto,node distance=4.5cm, semithick, scale=0.9]
      ...
      \node[state, myblue]          (C) at ( 4, -4) {\StObjIdsBlockingA};
      \node[state, myblue]          (F) at ( 0, -6) {\StObjIdsBlockingR};
      ...
      \draw (C)  edge[bend left=15]             node[fill = white, anchor = west]{\MsgAwaitReply}      (F);
      \draw (F)  edge[bend left=45]      node[fill = white, left = -4pt]{\MsgServerIdle}     (A);
      \draw (C)  edge[bend left=15]      node[fill = white, above = -6pt]{\MsgReplyObjIds}     (A);
      \draw (F)  edge[bend left=15]      node[fill = white, right = -16pt]{\MsgReplyObjIds}     (A);
    \end{tikzpicture}
  \end{center}
