...
%% `architecture/` prefix.
\lstinputlisting[style=cddl]{architecture/object-diffusion.cddl}
...
If the pipeline of that specific client-server pair contains only \MsgRequestObjs{} messages, and these messages span over all the objects in the FIFO, then send \MsgRequestObjIdsB. Otherwise, send \MsgRequestObjIdsNB.

Note that this criterion covers the simpler case of empty pipelines and FIFO.
Note that, if we send \MsgRequestObjIdsB{} while the messages in the pipeline do not cover the whole FIFO, then we might get stuck with ids that we will not be able to process until the server gets net ones.
