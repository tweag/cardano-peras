...
\begin{figure}[h]
  \begin{center}
    \begin{tikzpicture}[->,shorten >=1pt,auto,node distance=4.5cm, semithick, scale=0.9]
      ...
      \draw (D)  edge[bend left=45]      node[fill = white, anchor = center]{\MsgReplyObjIds}                  (A);
      \draw (A)  edge[bend left=15]      node[fill = white, below = 2pt]{\MsgRequestObjIdsNB} (D);
      \draw (A)  edge[bend right=45]      node[fill = white, above = 2pt]{\MsgRequestObjs}     (E);
      \draw (E)  edge[bend right=45]      node[fill = white, below = 2pt]{\MsgReplyObjs}       (A);
