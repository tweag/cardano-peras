...
\begin{tikzpicture}[->,shorten >=1pt,auto,node distance=4.5cm, semithick]
  ...
  \node[state, mygreen]          (C) at ( 4, -4) {\StObjIdsBlocking};
  \node[state, mygreen]          (D) at (-4, -4) {\StObjIdsNonBlocking};
  \node[state, mygreen]          (E) at ( 0,  4) {\StObjs};
...
\begin{figure}[h]
  \begin{center}
    \begin{tabular}{l|l}
      ...
      \StInit             & \Client \\
      \StIdle             & \Server \\
      \StObjIdsBlocking    & \Client \\
      \StObjIdsNonBlocking & \Client \\
      \StObjs              & \Client \\
    \end{tabular}
