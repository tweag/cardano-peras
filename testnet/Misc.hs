...
firstNonEmptyLine tag =
  Stream.fold (fromMaybe (error [str|Empty: #{tag}|]) <$> Fold.one)
    . nonEmptyLines
...
cardanoCli :: FilePath
cardanoCli =
  fromMaybe "cardano-cli" $ unsafePerformIO (lookupEnv "CARDANO_CLI")

{-# NOINLINE cardanoNode #-}
cardanoNode :: FilePath
cardanoNode =
  fromMaybe "cardano-node" $ unsafePerformIO (lookupEnv "CARDANO_NODE")

{-# NOINLINE cardanoTestnet #-}
cardanoTestnet :: FilePath
cardanoTestnet =
  fromMaybe "cardano-testnet" $ unsafePerformIO (lookupEnv "CARDANO_TESTNET")
