...
### Why to we need votes?

In order to keep the information about the last succesful round we need to keep a proof of that.
...
because all Peras certs are off-chain data.
...
to contain consensus logic or use callbacks, or ledger do not validate all the logic. If in the future we will consider keeping Peras certificates in the header it will lead to a very substantial refactoring. In case we keep code in consensus, the refactoring
will be much easier.
